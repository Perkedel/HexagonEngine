...
@export var list_menu_container_path:NodePath
...
@export var list_dialog_path:NodePath
...
func _input(event):
	if event is InputEventMouseMotion:
		var _left_boundary = list_dialog.global_position.x
		var _right_boundary = list_dialog.global_position.x + list_dialog.size.x
		var _top_boundary = list_dialog.global_position.y
		var _bottom_boundary = list_dialog.global_position.y + list_dialog.size.y
...
func add_menu(_list, _card):
	var _menu_button = menu_button_scene.instantiate()
	list_menu_container.add_child(_menu_button)
	_menu_button.text = _list.list_name_label.text
	_menu_button.connect("pressed",Callable(self,"_on_MenuButton_pressed").bind(_list, _card),CONNECT_DEFERRED)
