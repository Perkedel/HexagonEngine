...
@export var project_board_name_input_path : NodePath
...
@export var project_board_description_textarea_path:NodePath
...
func show_alert(_text: String, _variant: String="danger")->void:
	...
	_alert.connect("closed",Callable(self,"_on_Alert_closed").bind(),CONNECT_DEFERRED)
	alert_container.add_child(_alert)
#	_alert.start(Callable(_text,_variant))
